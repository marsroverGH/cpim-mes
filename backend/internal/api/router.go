...
func NewRouter(s *service.Services) http.Handler {
	...
	r.Route("/api", func(api chi.Router) {
		...
		api.Group(func(secured chi.Router) {
			...
			secured.Get("/dispatch", srv.currentDispatch)
			secured.Get("/schedule-execution", srv.currentScheduleExecution)
			secured.Get("/dispatch-policy/defaults", srv.dispatchPolicyDefaults)
			secured.Get("/dispatch-policy/current", srv.currentDispatchPolicy)
			secured.Get("/dispatch-policy-versions", srv.listDispatchPolicies)
			secured.With(requirePermission(PermDispatchManage)).Post("/dispatch-policy-versions", srv.createDispatchPolicy)
			secured.With(requirePermission(PermDispatchManage)).Post("/dispatch-policy-versions/{id}/activate", srv.activateDispatchPolicy)
			secured.Get("/schedule-adherence/current", srv.currentScheduleAdherence)
			secured.Get("/schedule-adherence/snapshots", srv.listScheduleAdherenceSnapshots)
			secured.Get("/schedule-adherence/snapshots/{id}", srv.getScheduleAdherenceSnapshot)
			secured.With(requirePermission(PermDispatchManage)).Post("/schedule-adherence/snapshots", srv.snapshotScheduleAdherence)
			secured.Get("/dynamic-rescheduling/runs", srv.listDynamicRescheduleRuns)
			secured.Get("/dynamic-rescheduling/runs/{id}", srv.getDynamicRescheduleRun)
			secured.Get("/dynamic-rescheduling/signals", srv.listRescheduleSignals)
			secured.With(requirePermission(PermDynamicReschedule)).Post("/dynamic-rescheduling/run", srv.runDynamicReschedule)
			secured.With(requirePermission(PermDynamicReschedule)).Post("/dynamic-rescheduling/process-pending", srv.processPendingReschedule)
