...
func TestRBACCriticalPermissions(t *testing.T) {
	tests := []struct {
		...
		{"operator cannot refresh Control Tower", domain.RoleOperator, PermControlTowerRefresh, false},
		{"planner can refresh Control Tower", domain.RolePlanner, PermControlTowerRefresh, true},
		{"operator cannot manage Control Tower", domain.RoleOperator, PermControlTowerManage, false},
		{"planner can manage Control Tower", domain.RolePlanner, PermControlTowerManage, true},
		{"operator cannot manage recovery scenario", domain.RoleOperator, PermRecoveryScenarioManage, false},
		{"planner can manage recovery scenario", domain.RolePlanner, PermRecoveryScenarioManage, true},
		{"operator cannot simulate recovery scenario", domain.RoleOperator, PermRecoverySimulationRun, false},
		{"planner can simulate recovery scenario", domain.RolePlanner, PermRecoverySimulationRun, true},
		{"operator cannot publish recovery scenario", domain.RoleOperator, PermRecoveryPublish, false},
		{"planner can publish recovery scenario", domain.RolePlanner, PermRecoveryPublish, true},
