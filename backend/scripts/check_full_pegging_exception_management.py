...
checks = {
    ...
    'Purchase Order supply is traced': ('purchase_orders' in svc or 'v_purchase_order_planning_schedule' in svc) and '"PURCHASE_ORDER"' in svc and '"PURCHASED_BY"' in svc,
    'supplier BLOCKED root cause is traced': ('supplier_quality_profiles' in svc or 'SupplierQualityStatus' in svc) and '"SUPPLIER_BLOCKED"' in svc,
    ...
    'pegging horizon limits future formal supply': all(x in svc for x in ['horizonEnd time.Time','w.due_date <= $2','d.due_at <= $2']) and ('p.due_date <= $2' in svc or 'p.expected_delivery_date <= $2' in svc),
